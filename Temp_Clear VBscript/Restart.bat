...
echo Scanning and deleting files in %temp%...
echo Scanning and deleting files in %temp%... >> "%logFilePath%"
for %%F in (%temp%\*.*) do (
    echo Deleting file: %%F >> "%logFilePath%"
    del /q /f "%%F" >> "%logFilePath%" 2>&1
)

:: Delete folders in %temp% and log them
for /d %%D in (%temp%\*) do (
...
echo Restarting the computer in 4 seconds... 
timeout /t 4 >nul
